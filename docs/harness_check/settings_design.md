...
|---|---|---|---|
| `SessionStart` | (なし) | `tools/hooks/load_session_context.py` | スタブ（B-2 で実体化）|
| `PreToolUse` | `Bash` | `tools/hooks/check_merge_order_exists.py` | スタブ |
| `PreToolUse` | `Bash` | `tools/hooks/check_task_progression.py` | スタブ |
| `PostToolUse` | `Bash` | `tools/hooks/validate_pptx_after_fill.py` | スタブ |

詳細は `tools/hooks/README.md` 参照。
...
- B-2 で hook スクリプトを実体化したら、本ドキュメントの「状態」欄を「実装済」に更新
- B-3 で `agents/research-subagent.md` を導入する場合、subagent 関連の permissions / env を追加検討
- B-6 で 既存 orchestrator 3 本を改訂したら、それらが期待する `FACTORY_ROOT` 等を再確認
