...
PROFILES: dict[tuple, list[Callable]] = {
    ...
    ("shareholder-structure-pptx", "roleup"): [
        # shareholder-structure はネイティブテーブル 2 つ (株主 + 役員) +
        # セクションタイトル textbox の構成、chart object 不在のため
        # C10/C12 を除外。残り 8 ルール (C1/C2/C4/C5/C6/C7/C8/C11) を適用。
        r for r in _COMMON_ROLEUP_RULES
        if r not in (rule_chart_axis_font_size, rule_no_chart_builtin_legend)
    ],
    ...
    ("valuation-summary-pptx", "roleup"): [
        # valuation-summary は手描き shape (Rectangle/Line) + textbox 構成、
        # chart object 不在のため C10/C12 を除外。3 つの chart_type
        # (football_field / equity_bridge / financial_summary) すべて手描き。
        # 残り 8 ルール (C1/C2/C4/C5/C6/C7/C8/C11) を適用。
        r for r in _COMMON_ROLEUP_RULES
        if r not in (rule_chart_axis_font_size, rule_no_chart_builtin_legend)
    ],
    ...
    ("pyramid-structure-pptx", "roleup"): [
        # pyramid-structure は code 内で動的に Rectangle (各段) + TextBox (詳細) を配置する図形系スライド。
        # chart object 不在のため C10/C12 を除外、残り 8 ルール (C1/C2/C4/C5/C6/C7/C8/C11) を適用。
        r for r in _COMMON_ROLEUP_RULES
        if r not in (rule_chart_axis_font_size, rule_no_chart_builtin_legend)
    ],
    ("swot-pptx", "roleup"): [
        # swot は 4 象限 (S/W/O/T) を Rectangle ヘッダー + 薄背景 + bullet textbox で動的描画。
        # chart object 不在のため C10/C12 を除外、残り 8 ルール (C1/C2/C4/C5/C6/C7/C8/C11) を適用。
        r for r in _COMMON_ROLEUP_RULES
        if r not in (rule_chart_axis_font_size, rule_no_chart_builtin_legend)
    ],
    ("value-chain-pptx", "roleup"): [
        # value-chain は Chevron 5-7 個 + Rectangle (利益プールバー) + bullet textbox の図形系。
        # chart object 不在のため C10/C12 を除外、残り 8 ルール (C1/C2/C4/C5/C6/C7/C8/C11) を適用。
        r for r in _COMMON_ROLEUP_RULES
        if r not in (rule_chart_axis_font_size, rule_no_chart_builtin_legend)
    ],
    ("value-chain-matrix-pptx", "roleup"): [
        # value-chain-matrix は Chevron 5-7 個 + 4 行のマトリクス (記号セル + プレーヤーバー Rectangle)
        # + 水平セパレーター connector の図形系。chart object 不在のため C10/C12 を除外、
        # 残り 8 ルール (C1/C2/C4/C5/C6/C7/C8/C11) を適用。
        r for r in _COMMON_ROLEUP_RULES
        if r not in (rule_chart_axis_font_size, rule_no_chart_builtin_legend)
    ],
    ...
    ("pyramid-structure-pptx", "stellar_aiz"): [],  # TODO: ISSUE-010
