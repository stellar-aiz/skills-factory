"""One-shot generator: current-period-forecast-pptx の roleup curated template を
stella template から派生して作る。

実行: `python3 scripts/build_roleup_template.py` で
`assets/roleup/current-period-forecast-template.pptx` を上書き保存する。

設計:
  - スライドサイズ: A4 landscape (10691813 × 7559675 EMU)
  - Title 1 / Text Placeholder 2 / Content Area / Source 3 の座標調整
  - Source → Source 3 にリネーム + 6pt Yu Gothic UI 茶系
"""
import os
...
SRC = os.path.join(SKILL_DIR, "assets", "stellar_aiz", "current-period-forecast-template.pptx")
DST = os.path.join(SKILL_DIR, "assets", "roleup", "current-period-forecast-template.pptx")
