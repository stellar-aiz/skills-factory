"""One-shot generator: issue-risk-list-pptx の roleup curated template を
...
設計:
  ...
  - サンプル行 (TextBox 4/5/23/25/26 + Group 8/15/57/64/71/78 + Connector 28/7/51/63/70/77)
    は fill_issue_risk.py が動的に削除する設計だが、roleup template では事前に削除しておく
  - Source 3: 下端に新規追加
  - フォント: Yu Gothic UI
  - think-cell OLE は削除
"""
...
SRC = os.path.join(SKILL_DIR, "assets", "stellar_aiz", "issue-risk-list-template.pptx")
DST = os.path.join(SKILL_DIR, "assets", "roleup", "issue-risk-list-template.pptx")
...
# fill_issue_risk.py の REMOVE_NAMES と同じ集合 (template から事前に消しておく)
REMOVE_NAMES = {
...
def main():
    ...
    # サンプル行・サンプルヘッダー shape 群を削除 (fill 側で動的構築するため)
