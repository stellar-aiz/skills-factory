...
| 17 | `five-forces-pptx` | ✅ 完了 | A (図形系、テンプレ shape 比率スケール) | `f17a9cc`(2026-05-06) |
| 18 | `pyramid-structure-pptx` | ✅ 完了 | A (図形系、code 動的描画 + テンプレ命名統一) | `c2111ef`(2026-05-06) |
...
~~**残 ISSUE-010 タスク**: `value-chain-matrix-pptx` の brand-aware 化~~
→ **完了 (2026-05-06、commit `79ee4c2`)**。company-deepdive × roleup E2E warning=0 達成。
他の戦略フレームワーク系 (swot / five-forces / value-chain / pyramid-structure 等) も同期間に完了済。

### 戦略フレームワーク系 3 件完了 (2026-05-06、後続セッション)

ユーザー判断 (「戦略フレームワーク系 5 件まとめて」) を受けて、軽量から順に着手:
- ✅ `five-forces-pptx` (commit `f17a9cc`、図形系、テンプレ shape 比率スケール)
- ✅ `pyramid-structure-pptx` (commit `c2111ef`、code 動的描画、テンプレ命名 `pyramid-template-universal.pptx` → `pyramid-structure-template.pptx` に統一)
- ✅ `swot-pptx` (commit `6d17edf`、4 象限 code 動的描画、SWOT 4 色は universal indicator として両 brand 維持)

**累積 compliance check (2026-05-06)**: **29 PPTX × 246 checks 全 PASS** (前回 27 × 230 → +2 PPTX × +16 checks)
- 既存 brand-aware 27 (cp/me/ch + market 系 5 + sd/toc/da + exec + BDD trio + Pattern A 7 + Pattern C 3) + 新 3 件 (five-forces / pyramid-structure / swot)
- valuation-summary は smoke fail (sample_data の chart_type ハンドリング起因) のため除外、本件とは別 issue (個別 fill 自体は brand-aware 化済 commit `(next commit)(2026-05-06)`)

**前提となった事故対応**: 着手時に **`import os` 抜け bug が 13 fill に残存** していることを発見 (Phase 1.5 AST migration `28ed496` の混入トラブル、valuation-summary と同種)。スコープ外だが mechanical な 1-line fix のため一括対応 (commit `66faca7`、13 件)。

**残 戦略フレームワーク系 (完了、2026-05-06、後続セッション)**:
- ✅ `value-chain-pptx` (commit `4cfe93e`、テンプレ命名統一 `value-chain-pptx-template.pptx` → `value-chain-template.pptx`、シェブロン色を chart_palette 茶系グラデーションで構成、profit/position 色は universal indicator 維持)
- ✅ `value-chain-matrix-pptx` (commit `79ee4c2`、Chevron + 4 行マトリクス、Arial→Yu Gothic UI 統一で C8 修正、副次的に nested span bars の C11 overlap も sub-stripe 配置で修正)

**戦略フレームワーク系 5 件全て brand-aware 化完了** + **company-deepdive-agent × roleup E2E warning=0 達成** (2026-05-06、E2E 15/15 fills、brand_fallback=0)。

**累積 compliance check (2026-05-06)**: 31 PPTX × 262 checks 全 PASS (前回 29 × 246 → +2 PPTX × +16 checks)
ISSUE-010 Phase 2 の主要レイヤー終結。残 fill (BDD 系の一部、Pattern C smallcap 系等) は Phase 2 後半 / Phase 3 で順次対応。
...
2026-05-06 の market-overview-agent 実行（AD/ADAS Tier1 業界調査）で、P7 の positioning-map スライドが「成功ログを出しつつ実質空に近い」状態で出力された。原因は、オーケストレーター（Claude）が想像で書いた `data_07_positioning.json` のキー名・値スケールが `fill_positioning_map.py` の期待スキーマと一致していなかったが、`fill_positioning_map.py` がスキーマ検証をせず、不明キーを silent に無視して 5 プレイヤー配置の成功メッセージを返したこと。ユーザーが PowerPoint で開いて初めて気付いた。

具体的な不一致:
- `x_axis.left_label/right_label` を渡したが実際は `x_axis.low/high`（同様に y_axis）
- プレイヤー座標を 0-100 スケールで渡したが実際は 0-10（`x_axis.min/max` で定義）
- `players[].color_hint: "target_company"` を渡したが実際は `color: "#HEXコード"`
- `quadrants` / `implications` / `implications_title` / `chart_title` / `section_title` / `source` フィールドが**全て欠落**
- 結果: 軸ラベル両端・4 象限ラベル・示唆パネル・出典が空、プレイヤーは軸範囲外で右上隅に固まる

同セッションで初回エラーを出した 4 スキル（market-environment / market-share / competitor-summary / market-kbf）は ValueError で hard-fail したため、サンプルデータを参照して正しいスキーマで再生成できた。**positioning-map だけが silent fail だった**ため見落とした。

### 検討事項

- 全 PPTX スキルの fill スクリプトに「必須キーチェック」「想定外キー警告」を入れるか
- `references/sample_data.json` を**正本スキーマ**として位置付け、orchestrator が呼ぶ前に jsonschema 検証する仕組みを入れるか
- 各 fill スクリプトが受理可能な JSON スキーマを `references/json_schema.json`（JSON Schema 形式）として明示するか
- 共通ヘルパー（`skills/_common/lib/`）に「fill 前検証」関数を置くか

### 修正方針候補

- (a) **最小対応**: 各 fill スクリプトの `_validate_input()` に「想定外キー警告（stderr に WARN 出力）」と「必須キー欠落エラー（hard fail）」を追加。positioning-map にスキーマ検証ロジックを最優先で入れる
- (b) **中対応**: `references/sample_data.json` を全スキルで正本化し、各 fill スクリプトの冒頭で `_compare_keys_with_sample()` を呼んで diff を WARN 出力する
- (c) **大対応**: `references/json_schema.json` を JSON Schema として整備し、`tools/validate_data.py` を作成。orchestrator が fill_*.py を呼ぶ前に `python3 tools/validate_data.py --schema <skill>/references/json_schema.json --data <data.json>` で事前検証する

### 影響範囲（要点検対象）

- `skills/positioning-map-pptx/scripts/fill_positioning_map.py`（最優先）
- 他 PPTX スキルでも同じ silent fail の可能性あり。要点検候補:
  - executive-summary-pptx / table-of-contents-pptx / section-divider-pptx / data-availability-pptx（オプショナルフィールドが多いスキル）
- orchestrator 側: market-overview-agent / strategy-report-agent / company-deepdive-agent の Step 1 → Step 5 間にスキーマ検証ステップを追加するかも

### 参考ファイル

- `skills/positioning-map-pptx/scripts/fill_positioning_map.py`
- `skills/positioning-map-pptx/references/sample_data.json`
- 2026-05-06 実行ログ（market-overview-agent / AD-ADAS Tier1）

### 関連メモ

- ユーザー指摘: 「P7のポジショニングマップができてなかったです / それはなぜでしょうか？」（2026-05-06）
- 即時対応: data_07_positioning.json をサンプル準拠で書き直し → 再 fill → 再 merge で復旧（shape 数 21 → 29）。本 ISSUE は構造的再発防止のための起票
- 関連 ISSUE: ISSUE-009（research-subagent return value の規約遵守）と同じ「silent な規約逸脱」のパターンだが、こちらは subagent ではなく fill スクリプト側

### Phase 1 実装内容（2026-05-06）

方針 (a) 「fill 側 hard-fail + orchestrator 側運用ガード」を採用し、以下を実装：

**1. 共通ヘルパー新設**: `skills/_common/lib/validate_fill_input.py`
- `validate_fill_input(data, required_top, allowed_top, nested_required, per_item_required, skill_name)` を提供
- 必須キー欠落 → `ValueError` で hard-fail（受領キー一覧と sample_data.json 参照誘導付き）
- 想定外キー → stderr に WARN（タイポ・古いスキーマ流用検出）

**2. positioning-map-pptx に検証導入**: `skills/positioning-map-pptx/scripts/fill_positioning_map.py`
- main() で `json.load` 直後に `validate_fill_input` を呼ぶ
- 必須: `main_message / players / x_axis / y_axis`、x_axis/y_axis 内の `label/low/high`、players 各要素の `name/x/y`
- 動作確認 3 パターン PASS:
  - 旧スキーマ（`x_axis.left_label/right_label`、`players[].color_hint`） → ValueError で hard-fail
  - 想定外キー混入（`footer_source` / `implication_label`） → WARN 出力 + 処理継続
  - 正規スキーマ（修復済 data_07_positioning.json） → 従来通り成功

**3. 3 orchestrator SKILL.md に運用ガード追記**:
- `skills/market-overview-agent/SKILL.md` Step 5
- `skills/strategy-report-agent/SKILL.md` Step 7〜N
- `skills/company-deepdive-agent/SKILL.md` Step 5
- 内容: 「fill_*.py を呼ぶ前に sample_data.json を必ず Read。想像で JSON を書かない」

### Phase 2（次回以降の課題）

横展開対象（silent fail の可能性が残る PPTX スキル）:
- executive-summary-pptx / table-of-contents-pptx / section-divider-pptx / data-availability-pptx（オプショナルフィールドが多いスキル）
