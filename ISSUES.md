...
**Status**: 保留 / **Priority**: P2 / **Decided**: 2026-05-03 / **Discovery**: γ E2E (market-overview-agent / 国内タクシー市場)
...
## ISSUE-011: format_add ブランチ V2 前倒し完結（rollup 仕様正本化と Pilot 3 改修）

**Status**: 進行中 / **Priority**: P1 / **Decided**: 2026-05-04

### 背景
V1（commit `4d752b1` 〜 `0a811c8`）で Pilot 3 を brand-aware 化したが、ユーザーが `--brand rollup` 出力を確認し
「キーメッセージ・タイトル位置とフォント、コンテンツ配置、フォントサイズなど、ロールアップ仕様の根幹がほぼ反映できていない」と判定。
構造的未到達領域（curated rollup template 不在、theme.json 数値が stella 流用、行高/数値書式/条件付きフォントサイズ schema 未定義）が判明し、
...
| Phase | 内容 | 状態 |
|---|---|---|
| Phase 0 | 仕様正本化 + 公式テンプレ受領 | ✅ 完了（2026-05-04） |
| Phase 1 | theme.json schema 2.0 拡張 + brand_resolver accessor 追加 | ⏸ 次着手 |
| Phase 2 | curated rollup template 配置 + check_template_invariants.py | ⏸ |
| Phase 3 | layout.json A4 横座標再計算 | ⏸ |
| Phase 4 | fill スクリプト 3 件改修（行高/数値書式/条件フォント/出所必須/hex 漏れ） | ⏸ |
| Phase 5 | check_brand_compliance.py + 検証 | ⏸ |
...
### 成果物
- ✅ `skills/_common/brands/rollup/format_spec.md` 新規作成（vF 20250928 正本）
- ✅ 公式テンプレ `work/rollup_official_templates/standard_format_vF_20250928.pptx` 受領

### 関連ファイル
- 計画書: `/Users/nakamaru/.claude/plans/lovely-skipping-aho.md`
- 仕様正本: `skills/_common/brands/rollup/format_spec.md`
- 公式テンプレ: `work/rollup_official_templates/standard_format_vF_20250928.pptx`
...
`stellar_aiz` / `rollup` の出力切替機構を確立した。残り 25 fill scripts は brand-aware 化未対応。
...
- Rollup でのクライアント納品需要が顕在化（pilot 3 以外のスキルで Rollup 出力が必要になる）
...
### V2 で curated rollup template の導入も併せて検討
V1 では Rollup 用テンプレ pptx は配置せず、`brand_resolver.template_path()` のフォールバックで stella テンプレを流用。
V2 で各スキルに A4 横（11.69×8.27）+ Yu Gothic UI + 褐色アクセントの curated rollup テンプレを順次導入。

### 参考ファイル
- `skills/_common/lib/brand_resolver.py`
- `skills/_common/brands/{stellar_aiz,rollup}/theme.json`
