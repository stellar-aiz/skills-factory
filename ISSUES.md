...
| **Phase 4 残** | **追加微修正 + Phase 4b/4c 残作業** | **⏸ 次セッション** |
| Phase 5 | check_brand_compliance.py + 検証 | ⏸ |

### 次セッション着手点（2026-05-04 引き継ぎ）

**最新 commit**: `fbd02d7` (format_add ブランチ)
**最新出力**: `outputs/v2_phase4_align_final/cp_roleup.pptx`, `me_roleup.pptx`, `ch_roleup.pptx` (+ stella×3)

#### 既知の残タスク
1. **追加微修正**(ユーザー視覚レビューで指摘予定)
2. **Phase 4c 残**: market-environment の Source 3 placeholder 反映
   - 現状: fill_market_environment.py L78 の `SHAPE_SOURCE = "Source"` は roleup テンプレ shape "Source 3" と不一致 → "Source not found" warning が出ており source テキストが出力されない
   - 解決方針: customer-profile fill 同様、shape を順に "Source 3"→"Source" でフォールバック検索
3. **Phase 4b**: company-history のさらなる調整(必要であれば)
   - 現状: 概念整理 commit (6b85a3c) でメッセージ/サブタイトル swap 適用済
   - 行高 12pt / 数値書式 / 出所 placeholder 等の追加チューニングが残るかは視覚レビュー次第
4. **Phase 5**: `tools/check_brand_compliance.py` 新規作成、最終ユーザー視覚レビュー、ISSUE-011 クローズ
