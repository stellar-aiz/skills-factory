...
**残 戦略フレームワーク系 (2 件、次セッション)**:
- `value-chain-pptx` (442 行、テンプレ命名統一 `value-chain-pptx-template.pptx` → `value-chain-template.pptx` 要)
- `value-chain-matrix-pptx` (533 行、最大、E2E warning 発生源 — 完了で company-deepdive E2E warning=0 達成)

完了時点で **戦略フレームワーク系 5 件全て brand-aware 化** + **company-deepdive-agent × roleup E2E warning=0** が達成され、ISSUE-010 Phase 2 の主要レイヤーが終結する見込み。
